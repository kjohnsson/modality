...
def positive_Sk(h, data, k, lambda0, m0):
    ...
    for i, (ind_left, ind_right) in enumerate(zip(ind_loc_max[:-1], ind_loc_max[1:])):
        ...
        x_loc_min[i+1] = minimize(lambda x: np.exp(kde.score_samples(x)), x0=x0,
    ...
    for i, (pr_left, pr_right) in enumerate(zip(prob_x_loc_min[:-1], prob_x_loc_min[1:])):
        ...
        if np.isnan(a[i]):
            ...
            a[i] = brentq(lambda x: kde.score_samples(x) - np.log(lambdas[i]),
                          x_grid[ind_right-1], x_grid[ind_right], rtol=1e-6)
                          #x_loc_min[i], x_loc_max[i], rtol=1e-6)
        ...
        if np.isnan(b[i]):
            ...
            b[i] = brentq(lambda x: kde.score_samples(x) - np.log(lambdas[i]),
                          x_grid[ind_right-1], x_grid[ind_right], rtol=1e-6)
                          #x_loc_max[i], x_loc_min[i+1], rtol=1e-6)
    # plt.figure()
    # plt.plot(x_grid, np.exp(log_prob_x))
    # for a_i in a:
...
if __name__ == '__main__':
    ...
    np.random.seed(seed)
    dat = np.hstack([np.random.randn(100), np.random.randn(100)+4]).reshape(-1, 1)
    ...
    #print "fisher_marron_pval(dat, 1, 0.1, 0.01, B=10) = {}".format(fisher_marron_pval(dat, 1, 0.1, 0.01, B=100))
